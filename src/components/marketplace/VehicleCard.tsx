...
export function VehicleCard({ vehicle }: VehicleCardProps) {
  ...
  const path = getVehiclePath(vehicle.brand, vehicle.model, vehicle.version, vehicle.year, vehicle.id);
          ...
          <div className="flex sm:flex-col">
            ...
            <div className="relative w-36 shrink-0 min-h-[160px] sm:min-h-0 sm:w-full sm:aspect-[16/11] overflow-hidden bg-muted rounded-l-2xl sm:rounded-l-none sm:rounded-t-2xl">
              {hasPhoto ? (
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <Car className="h-10 w-10 sm:h-16 sm:w-16 text-primary/20" />
                </div>
              )}

              {/* Gradient overlay - desktop only */}
              <div className="hidden sm:block absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-60 group-hover:opacity-40 transition-opacity duration-500" />

              {/* Badges */}
              <div className="absolute top-2 left-2 sm:top-4 sm:left-4 flex flex-col gap-1">
                <div className="flex gap-1 flex-wrap">
                  {vehicle.isFeatured && (
                    <Badge className="bg-primary text-primary-foreground border-none font-bold tracking-tighter px-2 py-0.5 sm:px-3 sm:py-1 rounded-full shadow-lg shadow-primary/20 text-[8px] sm:text-[10px]">
                      <Star className="h-2.5 w-2.5 fill-current mr-0.5" /> DEST.
                    </Badge>
                  )}
                  {vehicle.isInspected && (
                    <Badge className="bg-blue-500 text-white border-none font-bold tracking-tighter px-2 py-0.5 sm:px-3 sm:py-1 rounded-full shadow-lg shadow-blue-500/20 text-[8px] sm:text-[10px]">
                      <CheckCircle2 className="h-2.5 w-2.5 mr-0.5" /> PER.
                    </Badge>
                  )}
                </div>
                <div className="flex gap-1">
                  <Badge className="bg-white/10 backdrop-blur-md text-white border-white/10 font-bold px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-[8px] sm:text-[10px]">
                    {vehicle.condition}
                  </Badge>
                  <Badge className="bg-yellow-500/20 text-yellow-500 border border-yellow-500/30 font-bold tracking-tighter px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-[8px] sm:text-[10px]">
                    B2B
                  </Badge>
                </div>
              </div>
              ...
              <div className="hidden sm:flex absolute bottom-4 left-4 right-4 justify-between items-end">
            ...
            <div className="flex-1 min-w-0 flex flex-col">
              ...
              <div className="sm:hidden px-3 pt-3 pb-1">
                <h3 className="font-bold text-sm leading-tight uppercase tracking-tighter truncate">
                  {vehicle.brand} {vehicle.model}
                </h3>
              ...
              <CardContent className="flex-1 p-3 sm:p-6 pt-0 sm:pt-6 space-y-2 sm:space-y-6">

                {/* Seller + location — desktop only */}
